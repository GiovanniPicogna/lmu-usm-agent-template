...
def parse_args():
    ...
    p.add_argument("--json", default=None,
                   help="JSON string of parameter overrides (see below).")
    ...
    p.add_argument("--partition", default="serial",
                   help="SLURM partition name.")
    p.add_argument("--walltime", default="04:00:00",
                   help="SLURM wall time (HH:MM:SS).")
    p.add_argument("--memory_gb", type=float, default=8.,
                   help="Memory per node in GB for SLURM.")

    # --- stellar ---
    p.add_argument("--star_mass_msun",   type=float, default=1.0)
    p.add_argument("--star_radius_rsun", type=float, default=2.0)
    p.add_argument("--star_teff",        type=float, default=5772.)

    # --- grid ---
    p.add_argument("--nr",       type=int,   default=100)
    p.add_argument("--rmin_au",  type=float, default=1.)
    p.add_argument("--rmax_au",  type=float, default=1000.)
    p.add_argument("--nmbpd",    type=int,   default=7,
                   help="Mass bins per decade; minimum 7.")
    ...
    p.add_argument("--sigma_rc_au",    type=float, default=30.)

    # --- dust ---
    p.add_argument("--d2g",            type=float, default=0.01)
    p.add_argument("--vfrag_cms",      type=float, default=100.,
                   help="Fragmentation velocity in cm/s (default 100 = 1 m/s).")
    p.add_argument("--rho_monomer",    type=float, default=1.67,
                   help="Monomer bulk density [g/cm³].")
    p.add_argument("--a_ini_max_cm",   type=float, default=1e-4,
...
KNOWN_JSON_KEYS = {
    "star_mass_msun", "star_radius_rsun", "star_teff",
    "nr", "rmin_au", "rmax_au", "nmbpd", "mmin_g", "mmax_g",
    "alpha", "mdisk_msun", "sigma_exp", "sigma_rc_au",
    "d2g", "vfrag_cms", "rho_monomer", "a_ini_max_cm",
    "t_start_yr", "t_end_yr", "snaps_per_decade",
}
...
def apply_json_overrides(args, json_str: str):
    ...
    if unknown:
        print(f"ERROR message=Unknown JSON keys: {sorted(unknown)}. "
              f"Known keys: {sorted(KNOWN_JSON_KEYS)}", flush=True)
        sys.exit(1)
...
def validate_params(args):
    errors = []
    if args.nmbpd < 7:
        errors.append(f"nmbpd={args.nmbpd} < 7 — required for accurate "
                      f"coagulation (Drążkowska et al. 2014)")
    if args.rmin_au <= 0 or args.rmax_au <= args.rmin_au:
        errors.append(f"Invalid grid: rmin={args.rmin_au} AU, "
                      f"rmax={args.rmax_au} AU")
    if args.t_end_yr <= args.t_start_yr:
        errors.append(f"t_end_yr={args.t_end_yr} must be > "
                      f"t_start_yr={args.t_start_yr}")
    if args.mmin_g <= 0 or args.mmax_g <= args.mmin_g:
        errors.append(f"Invalid mass grid: mmin={args.mmin_g} g, "
                      f"mmax={args.mmax_g} g")
...
def build_and_run(args):
    ...
    if args.dry_run:
        n_snaps_dry = int(args.snaps_per_decade *
                          np.log10(args.t_end_yr / args.t_start_yr)) + 1
        print(f"SUCCESS run_dir={run_dir} dry_run=True "
    ...
    if args.resume:
        ...
        if not os.path.exists(dump_path):
            print(f"ERROR message=Resume requested but dump file not found: "
    ...
    sim.t.snapshots = np.hstack([
        sim.t,
        np.geomspace(args.t_start_yr, args.t_end_yr,
                     num=int(args.snaps_per_decade *
                             np.log10(args.t_end_yr / args.t_start_yr)) + 1
                     ) * c.year
    ])
    ...
    if n_snaps == 0:
        print(f"ERROR message=Run completed but no HDF5 files found in "
              f"{datadir}", flush=True)
        sys.exit(1)
    ...
    print(f"SUCCESS run_dir={run_dir} n_snapshots={n_snaps} "
...
def main():
    ...
    if args.hpc:
        ...
        if args.dry_run:
            print(f"SUCCESS run_dir={os.path.abspath(args.run_dir)} "
                  f"slurm_script={slurm_path} dry_run=True", flush=True)
        else:
            import subprocess
            ...
            if result.returncode != 0:
                print(f"ERROR message=sbatch failed: {result.stderr.strip()}",
                      flush=True)
                sys.exit(1)
