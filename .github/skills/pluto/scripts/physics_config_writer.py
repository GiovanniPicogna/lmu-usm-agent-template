...
def _parse_definitions_h(run_dir: str) -> dict:
    ...
    for key in [
        "PHYSICS", "DIMENSIONS", "GEOMETRY",
        "COMPONENTS",
        "EQUATION_OF_STATE",
        "TIME_STEPPING",
        "RECONSTRUCTION",
        "DIVB_CONTROL",
        "RESISTIVITY", "VISCOSITY", "THERMAL_CONDUCTION",
        "BODY_FORCE", "COOLING",
        "ROTATING_FRAME",
        "PARTICLES",
        "USER_DEF_PARAMETERS",
        "UNIT_DENSITY", "UNIT_LENGTH", "UNIT_VELOCITY",
        "GAMMA",
        "UNIT_DENSITY_LABEL", "UNIT_LENGTH_LABEL", "UNIT_VELOCITY_LABEL",
    ]:
        if key in raw:
            result[key] = raw[key]
...
def write_physics_config(run_dir: str, extra: Optional[dict] = None) -> str:
    ...
    physics_keys = [
        "PHYSICS", "DIMENSIONS", "GEOMETRY", "COMPONENTS",
        "EQUATION_OF_STATE", "TIME_STEPPING", "RECONSTRUCTION",
        "DIVB_CONTROL", "RESISTIVITY", "VISCOSITY", "THERMAL_CONDUCTION",
        "BODY_FORCE", "COOLING", "ROTATING_FRAME", "PARTICLES",
        "USER_DEF_PARAMETERS", "GAMMA",
    ]
    ...
    if extra:
        ...
        for k, v in extra.items():
            if k not in cfg or k in ("arch", "config_num", "binary",
                                     "parallel", "modules", "compiled_at",
                                     "skill_version"):
                lines.append(f"{k} = {v}")
...
def main() -> None:
    p = argparse.ArgumentParser(
        description="Parse definitions.h and write physics_config.md."
    )
