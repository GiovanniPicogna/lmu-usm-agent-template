...
@pytest.mark.parametrize(
    "hook_name", sorted(EXPECTED_HOOKS), ids=lambda n: n.replace(".sh", "")
)
...
def test_pre_bash_safety_blocks_rm_rf():
    ...
    assert "rm -rf" in content or "rm_rf" in content.lower(), (
...
def test_pre_bash_safety_blocks_force_push():
    content = (HOOKS_DIR / "pre-bash-safety.sh").read_text()
    assert "--force" in content or "force" in content.lower(), (
...
def test_pre_bash_safety_blocks_hpc_submission():
    ...
    assert found, (
        f"pre-bash-safety.sh must check for HPC submission commands ({hpc_cmds})"
    )


def test_pre_bash_safety_emits_valid_json_schema():
    content = (HOOKS_DIR / "pre-bash-safety.sh").read_text()
    assert "permissionDecision" in content or "hookSpecificOutput" in content, (
...
def test_python_instructions_covers_key_conventions():
    ...
    for mention in required_mentions:
        assert mention in content, (
            f"python.instructions.md must mention '{mention}'"
        )
