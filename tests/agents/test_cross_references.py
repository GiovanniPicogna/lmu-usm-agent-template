...
PIPELINE_AGENTS = {
    "hypothesis-agent", "analytical-agent", "setup-agent",
    "simulation-agent", "analysis-agent", "interpretation-agent",
    "paper-agent", "literature-agent", "spectral-agent",
    "retrieval-agent", "mcmc-agent",
}
...
def test_all_pipeline_agents_have_agent_files():
    ...
    assert not missing, (
        f"Agents listed in ARCHITECTURE.md have no .agent.md files: {missing}"
    )
...
def test_architecture_references_all_expected_schemas():
    ...
    for schema in KNOWN_SCHEMAS:
        assert schema in arch, (
            f"Schema {schema!r} not referenced in ARCHITECTURE.md"
        )
...
def test_handoff_schemas_md_has_validation_rules_for_each_schema():
    ...
    for schema in KNOWN_SCHEMAS:
        ...
        assert "Validation rules" in section_text or "**Validation rules" in section_text, (
...
def test_skills_mentioned_in_architecture_have_directories():
    ...
    for skill in set(skill_names):
        assert (SKILLS_DIR / skill).exists(), (
            f"Skill `{skill}` referenced in ARCHITECTURE.md but no directory at {SKILLS_DIR / skill}"
        )
...
def test_claude_md_references_handoff_schemas():
    content = CLAUDE_MD.read_text()
    assert "handoff_schemas.md" in content or "shared" in content, (
...
def test_claude_agents_symlinks_cover_all_specialist_agents():
    ...
    assert not missing, (
        f"Specialist agents not symlinked into .claude/agents/: {missing}"
    )
